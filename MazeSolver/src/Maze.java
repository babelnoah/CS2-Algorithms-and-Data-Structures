...
public class Maze {
            ...
            for (int row=0; row<this.numRows; row++) {
                ...
                for (int col=0; col<this.numCols; col++) {
                    ...
                    if (line.charAt(col) == '#') {
                        ...
                        this.mazeGrid[row][col].setVisited(true);
        ...
        for (int row=0; row<this.numRows; row++) {
            for (int col=0; col<this.numCols; col++) {
                if (this.mazeGrid[row][col].isWall()) {
                    ...
                    System.out.print("B");
                } else if (this.mazeGrid[row][col].isVisited()){
                    System.out.print(".");
                } else {
                    System.out.print(" ");
                }
    ...
    public boolean isValidCell(int row, int col) {
        // TODO: Determine if the cell is valid to be visited
        if((row >= 0 && row < numRows) && (col >= 0 && col < numCols)){
            MazeCell c = getCell(row, col);
            return !c.isVisited() && !c.isWall();
        }
        return false;
    }
}
